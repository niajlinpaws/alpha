...
router.post('/notifications/send/',FX.adminAuth,(req, res, next)=>{
    ...
    let {SNS_USER_TOPIC,SNS_BUSINESS_TOPIC}=process.env;
    ...
    Notification.create(notification,(err)=>{
        ...
        User.updateMany(query,{$inc:{unreadCount:1}},(err,result)=>{
            ...
            if(receiverType === "all")
            {
                FX.send_push_notification(data,SNS_BUSINESS_TOPIC,(err)=>{
                    if(err)return next(err); 
                });

                FX.send_push_notification(data,SNS_USER_TOPIC,(err)=>{
                    if(err)return next(err); 
                });
            ...
            {
                let topic=(receiverType === "businessUser") ? SNS_BUSINESS_TOPIC : SNS_USER_TOPIC;
